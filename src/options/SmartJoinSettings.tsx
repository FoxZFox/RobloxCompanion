    ...
    <Section title="Smart Join">
      ...
      */}
      ...
      <div className="rc-field">
        ...
        <span className="rc-header__sub">
          Population · Reputation (your own flags) · Health (the server's FPS and how its
          own players are faring on it) · Freshness (when we first saw the server) ·
          Favourites. A signal that cannot be judged for a given server is left out of its
      ...
      */}
      <div className="rc-banner">
        <span>
          <strong>Server region is not available.</strong> Roblox publishes no endpoint that
          reports where a server runs. The one call that reveals it is the game client's own
          join request, which refuses browser traffic (<code>status 12</code>). Making it work
          would mean impersonating the Roblox client, which this extension will not do. It
